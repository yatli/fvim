...
module ModelImpl =
    ...
    let (|Mouse|Special|Normal|ImeEvent|TextInput|Unrecognized|) (x: InputEvent) =
        ...
        | Key(_, Key.Oem1)              | Key(_, Key.Oem5)
        | Key(_, Key.Oem6)              | Key(_, Key.Oem7)
        | Key(_, Key.Oem8)              | Key(_, Key.Oem102)
        | Key(_, Key.AbntC1)            | Key(_, Key.AbntC2)
        | Key(_, Key.OemOpenBrackets)   | Key(_, Key.OemCloseBrackets)
        | Key(_, Key.System)            | Key(_, Key.OemAttn)
        | Key(_, Key.DbeAlphanumeric)   | Key(_, Key.OemFinish)
        | Key(_, Key.DbeKatakana)       | Key(_, Key.DbeHiragana)
        | Key(_, Key.OemCopy)           | Key(_, Key.DbeSbcsChar)
        | Key(_, Key.OemEnlw)           | Key(_, Key.OemBackTab)
        | Key(_, Key.DbeRoman)          | Key(_, Key.DbeNoRoman)
        | Key(_, Key.Attn)              | Key(_, Key.CrSel)
        | Key(_, Key.DbeEnterImeConfigureMode)| Key(_, Key.ExSel)
        | Key(_, Key.DbeFlushString)    | Key(_, Key.Play)
        | Key(_, Key.DbeCodeInput)      | Key(_, Key.DbeNoCodeInput)
        | Key(_, Key.Zoom)              | Key(_, Key.NoName)
        | Key(_, Key.DbeDetermineString)| Key(_, Key.DbeEnterDialogConversionMode)
        | Key(_, Key.Pa1)               | Key(_, Key.OemClear)
